...
router.delete(
  ...
  (req, res) => {
    Profile.findOne({ user: req.user.id }).then(profile => {
      ...
      // Index of id not found
      if (removeIndex == -1) {
        res.status(400).json({ err: 'No such experience id' });
      }
      // ID found
      else {
        // Splice out of array
...
router.delete(
  ...
  (req, res) => {
    Profile.findOne({ user: req.user.id }).then(profile => {
      ...
      // Index of id not found
      if (removeIndex == -1) {
        res.status(400).json({ err: 'No such education id' });
      }
      // ID found
      else {
        // Splice out of array
...
router.delete(
  ...
  (req, res) => {
    Profile.findOneAndDelete({ user: req.user.id })
      ...
      .catch(err => res.status(400).json({ error: 'Error deleting profile' }));
  }
);
